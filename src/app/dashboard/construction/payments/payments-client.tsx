...
import { Plus, Search, ArrowDownLeft, ArrowUpRight, Trash2, Edit, Download } from 'lucide-react'
import { formatCurrency, cn } from '@/lib/utils'
import { ErrorBoundary } from '@/components/ui/error-boundary'
import { exportCSV } from '@/lib/export-csv'
import type { ConstructionPayment } from '@/types/construction'
...
export function PaymentsClient({
  ...
  const openEdit = (p: ConstructionPayment) => {
    setForm({
  ...
  const bulkDelete = async () => {
    if (!confirm(`هل تريد حذف ${selected.size} تدفق؟`)) {
      return
    }
    setBulkLoading(true)
    const ids = Array.from(selected)
    await Promise.all(ids.map((id) => fetch(`/api/construction/payments/${id}`, { method: 'DELETE' })))
            ...
            onClick={() =>
              exportCSV(
                filtered.map((p) => ({
                  ...
                  النوع: p.type === 'incoming' ? 'وارد' : 'صادر',
                  الوصف: p.description,
                  المشروع: p.con_projects?.name || '',
                  'طريقة الدفع': METHOD_AR[p.payment_method] || p.payment_method,
                  المرجع: p.reference || '',
                  المبلغ: p.amount,
                })),
                [
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((p) => (
                <tr
                  key={p.id}
                  className={`border-t hover:bg-muted/20 transition-colors ${selected.has(p.id) ? 'bg-primary/5' : ''}`}
                >
