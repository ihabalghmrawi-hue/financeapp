...
import type { ConstructionWorker } from '@/types/construction'
...
export function WorkersClient({ workers: init, currency }: { workers: ConstructionWorker[]; currency: string }) {
  ...
  const filtered = workers.filter((w) => {
    ...
    return (
      (!q ||
        ...
        (JOB_TYPES[w.job_type] || w.job_type).toLowerCase().includes(q) ||
        (w.phone || '').includes(q)) &&
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        setWorkers((prev) => prev.map((w) => (w.id === editing.id ? { ...w, ...data } : w)))
      } else {
          ...
          {filtered.map((w) => (
            <div key={w.id} className="bg-card border rounded-xl p-4 space-y-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${STATUS_COLORS[w.status] || 'bg-gray-100 text-gray-600'}`}
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border rounded-lg px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/30"
                    >
                      {Object.entries(JOB_TYPES).map(([v, l]) => (
                        <option key={v} value={v}>
                          {l}
                        </option>
                      ))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setForm((f: any) => ({ ...f, daily_rate: e.target.value }))}
                      className="w-full border rounded-lg px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/30"
                    />
                  </div>
                  <div>
