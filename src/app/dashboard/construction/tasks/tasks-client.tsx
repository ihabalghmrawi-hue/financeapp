...
export function TasksClient({
  ...
  const projectMap = new Map<string, ConstructionTask[]>()
  const noProject: ConstructionTask[] = []
  ...
  const projectGroups: { name: string; tasks: ConstructionTask[] }[] = []
  ...
  const quickStatus = async (t: ConstructionTask, status: string, progress?: number) => {
                ...
                {byStatus.map((col) => (
                  <div key={col.status} className="space-y-3">
                    ...
                    >
                      ...
                      <span className="text-xs bg-white/50 px-1.5 py-0.5 rounded-full font-bold">
                        {col.tasks.length}
                      </span>
                    </div>
