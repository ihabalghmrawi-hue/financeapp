  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          className={`border rounded-xl p-4 ${summary.netProfit >= 0 ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800' : 'bg-orange-50 dark:bg-orange-950/20 border-orange-200 dark:border-orange-800'}`}
        >
          <div
          ...
          <p
            className={`text-xl font-bold ${summary.netProfit >= 0 ? 'text-blue-800 dark:text-blue-300' : 'text-orange-800 dark:text-orange-300'}`}
          >
          ...
          <tbody>
            {(projectSummary as any[]).map((p: any) => (
              <tr key={p.id} className="border-t hover:bg-muted/20">
                ...
                <td className={`px-4 py-3 text-left font-bold ${p.profit >= 0 ? 'text-blue-600' : 'text-orange-500'}`}>
                  {fmt(p.profit)}
                </td>
              </tr>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {(workers as any[]).map((w: any) => (
              <div key={w.id} className="border rounded-xl p-3 text-center">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${w.status === 'available' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                >
